...
public class ConfigMenuProvider implements IMenuProvider<Menu>, DirectoryListener.FileListener {
    ...
    private void applySlotFromKey(String key, ConfigSection section) {
        if (section.isSet("slot") || section.isSet("x") || section.isSet("y")) {
            return;
        }
        ...
        try {
            section.set("slot", Integer.parseInt(slot));
        } catch (NumberFormatException ex) {
            if (isCoordinateSlot(slot)) {
                section.set("slot", slot);
            }
        }
    }

    private boolean isCoordinateSlot(String slot) {
    ...
    public void loadItem(AbstractMenu menu, String name, ConfigSection section,
        ...
        Object slot = section.get("slot");
        if (withSlot && slot instanceof List<?>) {
            int index = 0;
            for (Object entry : (List<?>) slot) {
                if (entry == null) {
                    continue;
                }
                section.set("slot", entry instanceof Number ? entry : entry.toString());
                loadItem(menu, name + "." + ++index, section, false, action);
            }
            section.set("slot", slot);
            return;
        }
    ...
    public int parseTime(ConfigSection config, String key, int def) {
        ...
        if (o instanceof String || ConfigSection.isPrimitiveWrapper(o)) {
            Integer parsed = parseTime(o.toString());
            if (parsed != null) {
                return parsed;
            }
        }
    ...
    private Integer parseTime(String value) {
        ...
        int[] multipliers = {
                ...
                20 * 60 * 60, 20 * 60 * 60, 20 * 60 * 60, 20 * 60 * 60, 20 * 60 * 60
        };
        ...
        try {
            return (int) Math.round(Double.parseDouble(text) * multiplier);
        } catch (NumberFormatException ex) {
            return null;
        }
    }
