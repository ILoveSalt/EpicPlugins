			...
			if(args.length == 0){
				...
				if(coinplayer != null){
					sender.sendMessage(MessageManager.getInstance().command_coins_showownmoney.replace("[amount]"
							, EpicCoins.getInstance().format(coinplayer.getCoins())));
				}
				...
				if(args[0].equalsIgnoreCase("top")){
					...
					for(CoinPlayer coinplayers : CoinSystem.getInstance().getPlayerManager().getTopCoins(10)){
						...
						sender.sendMessage(MessageManager.getInstance().command_coins_top_list.replace("[rang]"
								...
								, EpicCoins.getInstance().format(coinplayers.getCoins())));
					}
					return;
				...
				sender.sendMessage(MessageManager.getInstance().command_coins_showothermoney.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(coinplayer.getCoins())));
			}else if(args.length == 2){
				if(args[0].equalsIgnoreCase("reset")){
					CoinPlayer coinplayer = getCoinPlayer(sender,args[1]);
					if(coinplayer == null){
						sender.sendMessage(MessageManager.getInstance().playernotfound.replace("[player]",CoinSystem.getInstance().getConfig().defaultColor+args[1]));
						return;
					}
					coinplayer.setCoins(CoinSystem.getInstance().getConfig().system_player_startcoin,CoinsUpdateCause.ADMIN);
					sender.sendMessage(MessageManager.getInstance().command_coins_set_sender.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(CoinSystem.getInstance().getConfig().system_player_startcoin)));
				...
				if(args[0].equalsIgnoreCase("pay")){
					...
					if(coinsender == null){
						sender.sendMessage(MessageManager.getInstance().playernotfound.replace("[player]",CoinSystem.getInstance().getConfig().defaultColor+sender.getName()));
						return;
					}
					...
					sender.sendMessage(MessageManager.getInstance().command_coins_pay_sender.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(amount)));
					Player player = Bukkit.getPlayer(args[1]);
					if(player != null) player.sendMessage(MessageManager.getInstance().command_coins_pay_receiver.replace("[amount]", EpicCoins.getInstance().format(amount)).replace("[player]",coinsender.getColor()+coinsender.getName()));
					...
					sender.sendMessage(MessageManager.getInstance().command_coins_set_sender.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(amount)));
					Player player = Bukkit.getPlayer(args[1]);
					if(player != null) player.sendMessage(MessageManager.getInstance().command_coins_set_receiver.replace("[amount]", EpicCoins.getInstance().format(amount)));
					...
					sender.sendMessage(MessageManager.getInstance().command_coins_add_sender.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(amount)));
					Player player = Bukkit.getPlayer(args[1]);
					if(player != null) player.sendMessage(MessageManager.getInstance().command_coins_add_receiver.replace("[amount]", EpicCoins.getInstance().format(amount)));
					...
					sender.sendMessage(MessageManager.getInstance().command_coins_remove_sender.replace("[player]",coinplayer.getColor()+coinplayer.getName()).replace("[amount]", EpicCoins.getInstance().format(amount)));
					Player player = Bukkit.getPlayer(args[1]);
					if(player != null) player.sendMessage(MessageManager.getInstance().command_coins_remove_receiver.replace("[amount]", EpicCoins.getInstance().format(amount)));
				}else sendHelp(sender);
			}else sendHelp(sender);
		});
		return true;
	...
	private CoinPlayer getCoinPlayer(CommandSender sender,String name){
		if(name.equalsIgnoreCase("@p")){
			...
			for(Player player : Bukkit.getOnlinePlayers()){
				if(player != sender && player.getWorld().equals(location.getWorld())){
	...
	public static class DummyAllCoinPlayer extends CoinPlayer{
		...
		public void addCoins(long coins, CoinsUpdateCause cause, String message) {
			for(Player player : Bukkit.getOnlinePlayers()){
				...
				if(coinPlayer != null){
					player.sendMessage(MessageManager.getInstance().command_coins_add_receiver.replace("[amount]", EpicCoins.getInstance().format(coins)));
		...
		public void removeCoins(long coins, CoinsUpdateCause cause, String message) {
			for(Player player : Bukkit.getOnlinePlayers()){
				CoinPlayer coinPlayer = CoinSystem.getInstance().getPlayerManager().getPlayer(player.getUniqueId());
				if(coinPlayer != null){
					player.sendMessage(MessageManager.getInstance().command_coins_remove_receiver.replace("[amount]", EpicCoins.getInstance().format(coins)));
		...
		public void setCoins(long coins, CoinsUpdateCause cause, String message) {
			for(Player player : Bukkit.getOnlinePlayers()){
				CoinPlayer coinPlayer = CoinSystem.getInstance().getPlayerManager().getPlayer(player.getUniqueId());
				if(coinPlayer != null){
					player.sendMessage(MessageManager.getInstance().command_coins_set_receiver.replace("[amount]", EpicCoins.getInstance().format(coins)));
