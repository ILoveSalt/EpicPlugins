package hgds.epicgrief.epiccoins.core.storage.storage.sql;
...
import hgds.epicgrief.epiccoins.core.storage.storage.sql.query.SelectQuery;
...
public abstract class SQLCoinStorage implements CoinStorage {
    ...
    public boolean connect() {
        if(!isConnected()){
            ...
            try {
                connect(config);
                this.table = new Table(this, "EpicCoins_players");// .create("PRIMARY KEY (`ID`)")
                createTable(table);
    ...
    public List<CoinPlayer> getTopPlayers(int maxReturnSize) {
        return this.table.query().executeAndGetResult("SELECT * FROM `EpicCoins_players` ORDER BY `EpicCoins_players`.`coins` DESC LIMIT " + maxReturnSize, result -> {
            List<CoinPlayer> players = new ArrayList<>();
            while (result.next()) {
                players.add(new CoinPlayer(result.getInt("ID"),
                        UUID.fromString(result.getString("uuid")),
